...
public class model_main
{
    ...
    Reader r = new Reader();
    public model_main(int sg) //LosBetrag, StartGuthaben, Gruppen
    {
        startGuthaben = sg;
/*
    ...
    public Karte gemeinschaftsKarteAusfuehren(){
    ...
    public Karte ereignisKarteAusfuehren(){
    ...
    public Karte karteAusfuehren(){ // bei JEDEM Zug aufrufen
        Karte k;
        switch(felder[spieler[aktuellerSpieler].getFeld()].getAktion()) {
            case 1:
            k = gemeinschaftsKarteAusfuehren();
            break;
            case 2:
            k = ereignisKarteAusfuehren();
            break;
            case 3:
            k = new Karte(-1, "Frei Parken! Du erhältst "+getGeldInderMitte(), -1);
            freiParken();
            break;
            case 4:
            k = new Karte(-2,"Gefängnis-Feld erwischt! Gehen Sie ins Gefängnis!", -1);
    ...
    public Feld getFeldAktuellerSpieler(){
        return felder[getFeldSpieler(getAktuellerSpieler())];
    }
    ...
    public void mitteEinzahlen(int betrag){
        geldInderMitte = geldInderMitte + betrag;
    }
    ...
    public int[] wuerfeln(){ // Wenn ergebnis[2] == 1 muss der Controller nochmal würfeln
        int[] tempErgebnis = spieler[aktuellerSpieler].ziehen(felder);
        int[] ergebnis = new int[tempErgebnis.length + 1];
        int[] mieteBerechnen = mieteBerechnen(ergebnis[0] + ergebnis[1]);

        for(int i = 0; i < tempErgebnis.length; i++){
            ergebnis[i] = tempErgebnis[i];
        }
        ergebnis[6] = mieteBerechnen[2]; // an wen

        if(mieteBerechnen[2] != -1){ // wenn Besitzer VORHANDEN, Herrschaften
            spieler[mieteBerechnen[1]].abheben(mieteBerechnen[0]);
            spieler[mieteBerechnen[2]].einzahlen(mieteBerechnen[0]);
        } else { // Kein Besitzer -> in die Mitte
            spieler[mieteBerechnen[1]].abheben(mieteBerechnen[0]);
            mitteEinzahlen(mieteBerechnen[0]);
        }
        return ergebnis;
    }
